...
public:
    ...
    virtual const BaseObjVar* getBaseMemObj() const = 0 ;
    ...
    /// Return type of the value
    inline virtual const SVFType* getType() const = 0;
...
public:
    ...
    GepObjVar(const BaseObjVar* baseObj, NodeID i, const APOffset& apOffset,
        ...
        : ObjVar(baseObj->getValue(), i, ty), apOffset(apOffset), base(baseObj)
    {
    }
    ...
    inline void setBaseNode(const BaseObjVar* bs)
    {
        this->base = bs;
    }
    ...
    inline virtual const SVFType* getType() const
    {
        return SymbolTableInfo::SymbolInfo()->getFlatternedElemType(base->getType(), apOffset);
    }
...
public:
    ...
    /// Constructor
    GlobalObjVar(NodeID i, const MemObj* mem, PNODEK ty = GlobalObjNode):
...
public:
    ...
    ConstantDataObjVar(NodeID i, const MemObj* m, PNODEK ty = ConstantDataObjNode)
        : BaseObjVar(m->getValue(), i, m->getObjTypeInfo(), ty)
    {
    }
...
public:
    ...
    DummyObjVar(NodeID i, const MemObj* m, PNODEK ty = DummyObjNode)
        : BaseObjVar(nullptr, i, m->getObjTypeInfo(), ty)
    {
    }
