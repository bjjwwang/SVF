...
std::deque<const FunObjVar*> AbstractInterpretation::collectProgEntryFuns()
{
    ...
    const bool singleEntry = Options::FunEntry() == FunEntryMode::SINGLE;
    ...
    if (singleEntry && entryFunctions.empty())
    {
        SVFUtil::errs() << SVFUtil::errMsg(
            "AE -fun-entry=single requires a program entry function, but main/svf.main was not found.\n");
        assert(false && "No program entry function found for -fun-entry=single");
        abort();
    }
...
/// Program entry - entry policy is selected by -fun-entry.
