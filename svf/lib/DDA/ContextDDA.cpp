...
{
    const BaseObjVar* objVar = SVFUtil::cast<BaseObjVar>(_pag->getGNode(_pag->getBaseObject(getPtrNodeID(var))->getId()));
    assert(objVar && "base object is null??");
    if (SVFUtil::isa<HeapObjVar, DummyObjVar>(objVar))
    {
        if (!objVar->getValue())
        {
        ...
        else if(const SVFBaseNode* gNode = objVar->getGNode())
        {
            if (const auto& node =
